...
ws.set_row(3, 20)
ws.set_row(4, 22)
ws.set_row(5, 24)   # [追加A] 顧客類型チェック行

for i,lbl in enumerate(["日　付","担当者名","お客様名","支　店"]):
    ws.write(3,2+i, lbl, hdr(NAVY,WHITE,8,ha="center"))
    ws.write(4,2+i, "",  rec(GRAY2))

# [追加A] 顧客類型チェック（パブコメNo.26対応：継続×変化なし→比較推奨対象外の可能性）
ws.merge_range("B6:C6",
    "【顧客類型】\n契約区分を記録",
    hdr(DNAVY,WHITE,8,ha="center"))
ws.merge_range("D6:F6",
    "  ①新規加入  ②継続（変化なし）  ③継続（変化あり）　"
    "※②の場合：顧客が前年同条件を主体的に指定→比較推奨対象外となり得る（パブコメNo.26）",
    f(bg=LYELLOW,fg=DARK,sz=8,wrap=True))
ws.write("G6","記録 →", hdr(RED,WHITE,8,ha="center"))
ws.write("H6","", rec(RED))
ws.data_validation("H6",{
    "validate":"list",
    "source":["①新規加入","②継続（変化なし）","③継続（変化あり）"],
    "dropdown":True
})
...
# スクリプト
ws.merge_range("B17:C17","■ 読み上げ③\n（意向把握・例示）", hdr(DGRN,"#B8DEC9",8,ha="center"))
# [追加B] 例示確認の記録注記（パブコメNo.21対応：意向が不明確でも「例示して確認した」記録が必要）
ws.merge_range("D17:H17",
    "【例示して意向を確認】以下①〜⑤を例示してお客様の重視事項を確認・記録してください。"
    "「お任せ」の場合も可能な限り重視項目を例示し意向形成を促すことが必要です（パブコメNo.21）。",
    f(bg="#D6ECD9",fg=DGRN,sz=8,bold=False,wrap=True,
      tc=DGRN,bc=DGRN,lc=DGRN,rc=DGRN,ts=1,bs=1,ls=1,rs=1))
...
# 重視事項ヘッダー [追加B] "例示して確認"であることを明示
ws.merge_range("B19:C19","例示→意向確認\n（担当者が記録）", hdr(DGRN,"#B8DEC9",8,ha="center"))
ws.merge_range("D19:F19","確認内容（例示項目）", hdr(DGRN,"#B8DEC9",8,ha="center"))
...
companies=[
    (RED,  LRED,
     "損保ジャパン\n「THEクルマの保険」",
     "【強み】故障補償特約（走行不能時・最大30万円）／自転車・交通用具搭乗中の事故を通院含め実費補償"
     "／つながるドラレコ（安全運転割引）\n"
     "【注意】車両限定型は人・自転車との衝突は補償外。車両ごと盗難でないと車内手荷物は対象外。",
     "保険料・故障補償・自転車補償を重視する方に。ケガの入院補償（差額ベッド代等）はMS・TNより手薄。"),
    (GREEN,LGRN,
     "三井住友海上\n「GKクルマの保険」",
     "【強み】人身傷害諸費用特約（入院・差額ベッド代補償）／傷害一時金倍額払特約（5日未満でも1万円）"
     "／車両限定型でも人・自転車衝突補償○／車内手荷物盗難補償○\n"
     "【ドラレコ】プレミアムドラレコ360°・駐車監視・見守り機能（月1,200円）",
     "ケガの補償が全社最強クラス。クルマの補償も限定型で充実。おクルマQQ隊でロード保険金請求不要。"),
    (BLUE, LBLUE,
     "東京海上日動\n「トータルアシスト」",
     "【強み】入院時選べるアシスト特約（差額ベッド代・自動付帯）／傷害一時金（5日未満も補償）"
     "／車両限定型で人・自転車衝突補償○／車内手荷物盗難補償○／故障損害特約（最大30万円）\n"
     "【ドラレコ】DAP（月500円と低価格）",
     "ケガ・クルマ両面バランス型。自転車補償は実費かつ通院○。ドラレコは500円と低価格。"),
]
...
# 推奨根拠記録（募集人が記録）
ws.merge_range("B44:C44","  📝 推奨根拠の記録\n（比較推奨規制・必須）", hdr(DNAVY,WHITE,9))
# [追加D] 推奨根拠定型文ドロップダウン（パブコメNo.16：「合理的かつ一定の具体性を有する基準・理由の記録」）
ws.merge_range("D44:F44","",
    f(bg=LBLUE,fg=NAVY,sz=8,bold=True,wrap=False,
      tc=NAVY,bc=NAVY,lc=NAVY,rc=NAVY,ts=2,bs=2,ls=2,rs=2))
ws.data_validation("D44",{"validate":"list",
    "source":[
        "①団体割引により保険料面で有利なため推奨",
        "②顧客の意向（補償重視）に照らし推奨",
        "③顧客の意向（保険料重視）に照らし推奨",
        "④保険料は最安でないが顧客のリスク内容に最も合致するため推奨",
        "⑤保険料重視だが最安社はリスク合致度低・次点保険料の当社を推奨",
        "⑥団体加入資格なし・個人契約商品を推奨",
        "⑦前年と状況変化なし・同条件での継続を推奨",
        "⑧顧客が保険会社を主体的に指定",
    ],
    "dropdown":True})
...
ws.data_validation("H44",{"validate":"list",
    ...
    "dropdown":True})
# [追加C] 商品差説明チェック（パブコメNo.25：差異限定的の旨を説明した記録）
ws.set_row(44,20,None,G1H)
ws.merge_range("B45:F45",
    "  □ 取扱商品間の保険料・補償内容の差異が限定的である旨をお客様に説明しました（パブコメNo.25対応）",
    f(bg=LBLUE,fg=NAVY,sz=9,bold=True,wrap=False,
      tc=GRAY2,bc=GRAY2,lc=GRAY2,rc=GRAY2,ts=1,bs=1,ls=1,rs=1))
ws.write("G45","確認 →", hdr(NAVY,WHITE,7,ha="center"))
ws.write("H45","", rec(NAVY))
ws.data_validation("H45",{"validate":"list",
    "source":["説明済み","説明不要（差異大・個別比較実施）"],
    "dropdown":True})
